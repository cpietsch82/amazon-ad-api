...
const accessTokenInstance = new AccessToken().getInstance();
...
class Advertising {
  ...
  constructor(config){
    ...
    accessTokenInstance.init(config, this._credentials);
  ...
  get access_token(){
    return accessTokenInstance.access_token;
  }
  ...
  async _validateAccessToken(){
    if (this._options.auto_request_token && !accessTokenInstance.access_token){
      await this.refreshAccessToken();
    }
    if (!accessTokenInstance.access_token){
      throw new CustomError({
  ...
  _addHeaders(add_scope){
    let headers = {
      'Authorization':'Bearer ' + accessTokenInstance.access_token,
  ...
  _isUnauthorized(json_res, is_v3 = false){
    ...
    // return ((!is_v3 && json_res.message === 'Unauthorized') || (json_res.code === 'UNAUTHORIZED' && json_res.details === 'HTTP 401 Unauthorized')); // CP
    return ((json_res.message === 'Unauthorized') || (json_res.code === 'UNAUTHORIZED' && json_res.details === 'HTTP 401 Unauthorized')); // CP
  }
  ...
  _validateDownloadSuccess(res){
    if (res.statusCode !== 200){
      ...
      if (json_res && json_res.Error){
        console.log('CUSTOM ERROR '+json_res.Error.Message);
        throw new CustomError({
  ...
  async _getReportRedirectURL(report_url){
    ...
    if (res.statusCode === 307){
      ...
      // Refresh access_token when expired/invalid and auto_request_token is true
      if (this._options.auto_request_token && res.statusCode === 401 && this._isUnauthorized(json_res)){
        console.log(`REFRESH ACCESS TOKEN BECAUSE UNAUTHORIZED!!! (_getReportUrl)`);
        await this.refreshAccessToken();
  ...
  async refreshAccessToken(){
    await accessTokenInstance.refreshAccessToken(); // CP
    this._access_token = accessTokenInstance.acces_token;
  }
